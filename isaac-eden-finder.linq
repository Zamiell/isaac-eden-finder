...
public static class TrinketType
{
    ...
    public const int TRINKET_HAIRPIN = 120;
    public const int TRINKET_WOODEN_CROSS = 121;
    public const int TRINKET_BUTTER = 122;
    public const int TRINKET_FILIGREE_FEATHERS = 123;
    public const int TRINKET_DOOR_STOP = 124;
    public const int TRINKET_EXTENSION_CORD = 125;
    public const int TRINKET_ROTTEN_PENNY = 126;
    ...
    public const int TRINKET_SHORT_FUSE = 133;
    public const int TRINKET_GIGANTE_BEAN = 134;
...
public static EdenItems CalculateEdenItems(uint dropSeed)
{
    ...
    var hearts = 0;
    var soulHearts = 0;
    // 0x63fa5d: [player+0x334].Next(5)==0 is the trinket/grab-bag branch
    if ((rng.Next() % 5) == 0)
    {
        // 0x63fa7e: Next(2)==0 → variant=0x15e (trinket), else variant=0x46 (grab bag)
        if ((rng.Next() % 2) == 0)
        {
            // 0x63fac0: Next() = entity seed for the trinket pickup
            var trinketSeed = rng.Next();
            trinket = GetTrinket(trinketSeed);
        }
        else
        {
            // Grab bag: still advances rng for the entity seed (variant=0x46)
            rng.Next();
        }
    }
